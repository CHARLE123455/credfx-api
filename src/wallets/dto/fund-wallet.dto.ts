...
export class FundWalletDto {
    @ApiProperty({ example: 50000, description: 'Amount to fund (in the specified currency)' })
    @IsNumber({ maxDecimalPlaces: 2 })
    @IsPositive()
    amount: number;

    @ApiPropertyOptional({ example: 'NGN', description: 'Currency to fund (defaults to NGN)' })
    @IsOptional()
