...
export class WalletService {
  ...
  async fundWallet(
    ...
    if (dto.idempotencyKey) {
      ...
      if (cached)
        return cached as { transaction: Transaction; newBalance: number };
    }
    ...
    try {
      ...
      const balanceRepo = queryRunner.manager.getRepository(WalletBalance);
  ...
  async tradeCurrency(
    ...
    const feeAmount = parseFloat(
      ((rawSourceAmount * feePercent) / 100).toFixed(6),
    );
  ...
  private async executeExchange(params: {
    ...
    try {
      ...
      const sourceBalance = await balanceRepo.findOne({
      ...
      let targetBalance = await balanceRepo.findOne({
        where: { walletId: wallet.id, currency: params.toCurrency },
        lock: { mode: 'pessimistic_write' },
      });

      if (!targetBalance) {
        targetBalance = balanceRepo.create({
          walletId: wallet.id,
      ...
      targetBalance.balance = parseFloat(
        (
          parseFloat(targetBalance.balance.toString()) + params.creditAmount
        ).toFixed(6),
      );
      await balanceRepo.save(targetBalance);
