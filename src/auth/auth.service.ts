...
export class AuthService {
  ...
  async register(dto: RegisterDto): Promise<{ message: string }> {
    ...
    try {
      ...
      const user = userRepo.create({ ...dto, otp: otpHash, otpExpiresAt });
      ...
      const wallet = walletRepo.create({ user: savedUser });
      const savedWallet = await walletRepo.save(wallet);

      const ngnBalance = balanceRepo.create({
        wallet: savedWallet,
        currency: 'NGN',
        balance: initialBalance,
      });
      await balanceRepo.save(ngnBalance);

      await queryRunner.commitTransaction();
      await this.mailService.sendOtp(dto.email, dto.firstName, otp);

      return {
        message: `Registration successful. An OTP has been sent to ${dto.email}`,
      };
    } catch (error) {
