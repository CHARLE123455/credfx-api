...
export class FxService {
    ...
    async getRates(baseCurrency = 'NGN'): Promise<{ base: string; rates: Record<string, number>; source: string; cachedAt?: string }> {
    ...
    async getSupportedPairs(): Promise<{ pairs: string[]; total: number }> {
        const pairs: string[] = [];
    ...
    private async fetchAndCacheRates(
        ...
        try {
            ...
            const payload = { base: baseCurrency, rates: filteredRates, fetchedAt: new Date().toISOString() };
            const cacheKey = `fx_rates:${baseCurrency}`;
            ...
            this.logger.warn(`FX API fetch failed. Attempting DB fallback. Error: ${(error as Error).message}`);
    ...
    private async fallbackToDbSnapshot(
        ...
        this.logger.warn(`Using stale DB snapshot from ${snapshot.fetchedAt.toISOString()}`);
