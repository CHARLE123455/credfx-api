...
const mockSnapshotRepo = { findOne: jest.fn(), save: jest.fn(), create: jest.fn((data) => data) };
...
describe('FxService', () => {
    ...
    beforeEach(async () => {
        mockedAxios.create = jest.fn().mockReturnValue({
            get: jest.fn().mockResolvedValue({
                data: {
                    result: 'success',
                    base_code: 'NGN',
                    conversion_rates: { NGN: 1, USD: 0.000625, EUR: 0.00057, GBP: 0.00048 },
                },
            }),
        });
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                FxService,
                { provide: getRepositoryToken(FxRateSnapshot), useValue: mockSnapshotRepo },
    ...
    describe('getRates', () => {
        it('returns cached rates from Redis without hitting external API', async () => {
            const cachedData = JSON.stringify({ base: 'NGN', rates: { USD: 0.000625 }, fetchedAt: new Date().toISOString() });
        ...
        it('falls back to DB snapshot when external API fails', async () => {
            mockRedisService.get.mockResolvedValue(null);
            const mockAxiosInstance = { get: jest.fn().mockRejectedValue(new Error('Network Error')) };
            (service as unknown as { httpClient: { get: jest.Mock } }).httpClient = mockAxiosInstance;
        ...
        it('throws ServiceUnavailableException when API fails and no DB snapshot exists', async () => {
            ...
            await expect(service.getRates('NGN')).rejects.toThrow(ServiceUnavailableException);
        });
    });
